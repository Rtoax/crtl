...
#include <crypto/bit/non-atomic.h>
...
{
	return !crtl_bitmap_empty(node_marks(node, mark), XA_CHUNK_SIZE);
}

static inline void node_mark_all(struct xa_node *node, xa_mark_t mark)
{
	crtl_bitmap_fill(node_marks(node, mark), XA_CHUNK_SIZE);
}
...
static void xas_squash_marks(const struct xa_state *xas)
{
	...
	do {
		...
		if (crtl_find_next_bit(marks, limit, xas->xa_offset + 1) == limit)
			continue;
		__set_bit(xas->xa_offset, marks);
		crtl_bitmap_clear(marks, xas->xa_offset + 1, xas->xa_sibs);
...
static void *xas_alloc(struct xa_state *xas, unsigned int shift)
{
	...
	XA_NODE_BUG_ON(node, shift > CRTL_BITS_PER_LONG);
	XA_NODE_BUG_ON(node, !list_empty(&node->private_list));
	node->shift = shift;
	node->count = 0;
	node->nr_values = 0;
		...
		unsigned long last, void *entry, gfp_t gfp)
{
	...
	do {
		xas_lock(&xas);
		if (entry) {
			unsigned int order = CRTL_BITS_PER_LONG;
			if (last + 1)
				order = __ffs(last + 1);
 ...
 */
int __xa_alloc(struct xarray *xa, unsigned int *id, void *entry,
 ...
 */
int __xa_alloc_cyclic(struct xarray *xa, unsigned int *id, void *entry,
		struct xa_limit limit, unsigned int *next, gfp_t gfp)
{
	unsigned int min = limit.min;
	int ret;
...
void xa_dump_index(unsigned long index, unsigned int shift)
{
	if (!shift)
		pr_info("%lu: ", index);
	else if (shift >= CRTL_BITS_PER_LONG)
		pr_info("0-%lu: ", ~0UL);
	else
