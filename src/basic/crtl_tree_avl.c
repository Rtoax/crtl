/*
 * crtl_avl_tree.c - intrusive, nonrecursive AVL tree data structure (self-balancing
...
crtl_avl_get_child(const struct crtl_avl_tree_node *parent, int sign)
{
	if (sign < 0)
		return parent->left;
	else
...
static _inline struct crtl_avl_tree_node *
crtl_avl_tree_first_or_last_in_order(const struct crtl_avl_tree_node *root, int sign)
{
	const struct crtl_avl_tree_node *first = root;

	if (first)
		while (crtl_avl_get_child(first, +sign))
			first = crtl_avl_get_child(first, +sign);
	return (struct crtl_avl_tree_node *)first;
}
...
{
	return crtl_avl_tree_first_or_last_in_order(root, -1);
}
...
static _inline struct crtl_avl_tree_node *
crtl_avl_tree_next_or_prev_in_order(const struct crtl_avl_tree_node *node, int sign)
{
	const struct crtl_avl_tree_node *next;

	if (crtl_avl_get_child(node, +sign))
		for (next = crtl_avl_get_child(node, +sign);
		     crtl_avl_get_child(next, -sign);
		     next = crtl_avl_get_child(next, -sign))
			;
	else
		for (next = crtl_avl_get_parent(node);
		     next && node == crtl_avl_get_child(next, +sign);
		     node = next, next = crtl_avl_get_parent(next))
			;
	return (struct crtl_avl_tree_node *)next;
}
...
{
	return crtl_avl_tree_next_or_prev_in_order(node, 1);
}
...
{
	const struct crtl_avl_tree_node *next = prev_parent;
 ...
 * so the compiler can remove the conditional.  */
static _inline void
crtl_avl_set_child(struct crtl_avl_tree_node *parent, int sign,
	      struct crtl_avl_tree_node *child)
{
	if (sign < 0)
		parent->left = child;
	else
...
static _inline void
crtl_avl_set_parent_balance(struct crtl_avl_tree_node *node, struct crtl_avl_tree_node *parent,
		       int balance_factor)
{
...
static _inline void
crtl_avl_set_parent(struct crtl_avl_tree_node *node, struct crtl_avl_tree_node *parent)
{
...
static _inline int
crtl_avl_get_balance_factor(const struct crtl_avl_tree_node *node)
{
 ...
 * (-1, 0, or 1).  */
static _inline void
crtl_avl_adjust_balance_factor(struct crtl_avl_tree_node *node, int amount)
{
	node->parent_balance += amount;
}

static _inline void
crtl_avl_replace_child(struct crtl_avl_tree_node **root_ptr,
		  struct crtl_avl_tree_node *parent,
		  struct crtl_avl_tree_node *old_child,
		  struct crtl_avl_tree_node *new_child)
{
	if (parent) {
 ...
 */
static _inline void
crtl_avl_rotate(struct crtl_avl_tree_node ** const root_ptr,
	   struct crtl_avl_tree_node * const A, const int sign)
{
	struct crtl_avl_tree_node * const B = crtl_avl_get_child(A, -sign);
	struct crtl_avl_tree_node * const E = crtl_avl_get_child(B, +sign);
	struct crtl_avl_tree_node * const P = crtl_avl_get_parent(A);

	crtl_avl_set_child(A, -sign, E);
	crtl_avl_set_parent(A, B);

	crtl_avl_set_child(B, +sign, A);
	crtl_avl_set_parent(B, P);

	if (E)
		crtl_avl_set_parent(E, A);

	crtl_avl_replace_child(root_ptr, P, A, B);
}
 ...
 *	crtl_avl_rotate(root_ptr, B, -sign);
 *	crtl_avl_rotate(root_ptr, A, +sign);
 *
 * See comment in crtl_avl_handle_subtree_growth() for explanation of balance
 * factor updates.
 */
static _inline struct crtl_avl_tree_node *
crtl_avl_do_double_rotate(struct crtl_avl_tree_node ** const root_ptr,
		     struct crtl_avl_tree_node * const B,
		     struct crtl_avl_tree_node * const A, const int sign)
{
	struct crtl_avl_tree_node * const E = crtl_avl_get_child(B, +sign);
	struct crtl_avl_tree_node * const F = crtl_avl_get_child(E, -sign);
	struct crtl_avl_tree_node * const G = crtl_avl_get_child(E, +sign);
	struct crtl_avl_tree_node * const P = crtl_avl_get_parent(A);
	const int e = crtl_avl_get_balance_factor(E);

	crtl_avl_set_child(A, -sign, G);
	crtl_avl_set_parent_balance(A, E, ((sign * e >= 0) ? 0 : -e));

	crtl_avl_set_child(B, +sign, F);
	crtl_avl_set_parent_balance(B, E, ((sign * e <= 0) ? 0 : -e));

	crtl_avl_set_child(E, +sign, A);
	crtl_avl_set_child(E, -sign, B);
	crtl_avl_set_parent_balance(E, P, 0);

	if (G)
		crtl_avl_set_parent(G, A);

	if (F)
		crtl_avl_set_parent(F, B);

	crtl_avl_replace_child(root_ptr, P, A, E);
 ...
 */
static _inline bool
crtl_avl_handle_subtree_growth(struct crtl_avl_tree_node ** const root_ptr,
			  struct crtl_avl_tree_node * const node,
			  struct crtl_avl_tree_node * const parent,
			  const int sign)
{
	int old_balance_factor, new_balance_factor;

	old_balance_factor = crtl_avl_get_balance_factor(parent);

	if (old_balance_factor == 0) {
		crtl_avl_adjust_balance_factor(parent, sign);
	...
	if (sign * crtl_avl_get_balance_factor(node) > 0) {
		 ...
		 *	balance(A) = 0
		 */
		crtl_avl_rotate(root_ptr, parent, -sign);

		/* Equivalent to setting @parent's balance factor to 0.  */
		crtl_avl_adjust_balance_factor(parent, -sign); /* A */

		/* Equivalent to setting @node's balance factor to 0.  */
		crtl_avl_adjust_balance_factor(node, -sign);   /* B */
	} else {
		 ...
		 *	balance(E) = 0
		 */
		crtl_avl_do_double_rotate(root_ptr, node, parent, -sign);
	}
...
{
	struct crtl_avl_tree_node *node, *parent;
	bool done;
	...
	parent = crtl_avl_get_parent(node);
	if (!parent)
		return;

	if (node == parent->left)
		crtl_avl_adjust_balance_factor(parent, -1);
	else
		crtl_avl_adjust_balance_factor(parent, +1);

	if (crtl_avl_get_balance_factor(parent) == 0)
	...
	do {
		...
		if (node == parent->left)
			done = crtl_avl_handle_subtree_growth(root_ptr, node,
							 parent, -1);
		else
			done = crtl_avl_handle_subtree_growth(root_ptr, node,
							 parent, +1);
 ...
 */
static _inline struct crtl_avl_tree_node *
crtl_avl_handle_subtree_shrink(struct crtl_avl_tree_node ** const root_ptr,
			  struct crtl_avl_tree_node *parent,
			  const int sign,
			  bool * const left_deleted_ret)
{
	...
	if (new_balance_factor == 0) {
		...
		node = crtl_avl_get_child(parent, sign);

		/* The rotations below are similar to those done during
		 * insertion (see crtl_avl_handle_subtree_growth()), so full
		...
		if (sign * crtl_avl_get_balance_factor(node) >= 0) {
			...
			if (crtl_avl_get_balance_factor(node) == 0) {
				/*
				...
				/* B: 0 => +1 (sign < 0)
				 * or 0 => -1 (sign > 0)  */
				crtl_avl_adjust_balance_factor(node, -sign);
			...
			node = crtl_avl_do_double_rotate(root_ptr, node,
						    parent, -sign);
		}
	}
	parent = crtl_avl_get_parent(node);
...
static _inline struct crtl_avl_tree_node *
crtl_avl_tree_swap_with_successor(struct crtl_avl_tree_node **root_ptr,
			     struct crtl_avl_tree_node *X,
			     bool *left_deleted_ret)
{
	struct crtl_avl_tree_node *Y, *ret;
	...
	if (!Y->left) {
		...
		*left_deleted_ret = false;
		...
		Q->left = Y->right;
		if (Q->left)
			crtl_avl_set_parent(Q->left, Q);
		Y->right = X->right;
		crtl_avl_set_parent(X->right, Y);
		ret = Q;
		*left_deleted_ret = true;
	}

	Y->left = X->left;
	crtl_avl_set_parent(X->left, Y);

	Y->parent_balance = X->parent_balance;
	crtl_avl_replace_child(root_ptr, crtl_avl_get_parent(X), X, Y);
 ...
 *
 * @node
 *	Pointer to the `struct crtl_avl_tree_node' embedded in the item to
 ...
 * crtl_avl_tree_node_set_unlinked().
 */
void
crtl_avl_tree_remove(struct crtl_avl_tree_node **root_ptr, struct crtl_avl_tree_node *node)
{
	...
	if (node->left && node->right) {
		...
		parent = crtl_avl_tree_swap_with_successor(root_ptr, node,
						      &left_deleted);
		...
		if (parent) {
			...
			if (child)
				crtl_avl_set_parent(child, parent);
		} else {
			if (child)
				crtl_avl_set_parent(child, parent);
			*root_ptr = child;
	...
	do {
		if (left_deleted)
			parent = crtl_avl_handle_subtree_shrink(root_ptr, parent,
							   +1, &left_deleted);
		else
			parent = crtl_avl_handle_subtree_shrink(root_ptr, parent,
							   -1, &left_deleted);
