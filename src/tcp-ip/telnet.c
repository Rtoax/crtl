...
/* telnet state codes */
enum crtl_telnet_state_t {
	CRTL_TELNET_STATE_DATA = 0,
	CRTL_TELNET_STATE_EOL,
	CRTL_TELNET_STATE_IAC,
	CRTL_TELNET_STATE_WILL,
	CRTL_TELNET_STATE_WONT,
	CRTL_TELNET_STATE_DO,
	CRTL_TELNET_STATE_DONT,
	CRTL_TELNET_STATE_SB,
	CRTL_TELNET_STATE_SB_DATA,
	CRTL_TELNET_STATE_SB_DATA_IAC
};
typedef enum crtl_telnet_state_t crtl_telnet_state_t;
...
struct crtl_telnet_t {
	...
	const crtl_telnet_telopt_t *telopts;
	/* event handler */
	crtl_telnet_event_handler_t eh;
#if defined(HAVE_ZLIB)
	/* zlib (mccp2) compression */
	z_stream *z;
#endif
	/* RFC1143 option negotiation states */
	struct crtl_telnet_rfc1143_t *q;
	/* sub-request buffer */
	char *buffer;
	...
	size_t buffer_pos;
	/* current state */
	enum crtl_telnet_state_t state;
	/* option flags */
	unsigned char flags;
...
typedef struct crtl_telnet_rfc1143_t {
	unsigned char telopt;
	unsigned char state;
} crtl_telnet_rfc1143_t;
...
static crtl_telnet_error_t _error(crtl_telnet_t *telnet, unsigned line,
		const char* func, crtl_telnet_error_t err, int fatal, const char *fmt,
		...) {
	crtl_telnet_event_t ev;
	char buffer[512];
	va_list va;
	...
	ev.type = fatal ? CRTL_TELNET_EV_ERROR : CRTL_TELNET_EV_WARNING;
	ev.err.file = __FILE__;
	ev.err.func = func;
	ev.err.line = line;
	ev.err.msg = buffer;
	telnet->eh(telnet, &ev, telnet->ud);
 ...
 * (decompression).  returns CRTL_TELNET_EOK on success, something else on
 * failure.
 */
crtl_telnet_error_t _init_zlib(crtl_telnet_t *telnet, int deflate, int err_fatal) {
	z_stream *z;
	int rs;

	/* if compression is already enabled, fail loudly */
	if (telnet->z != 0)
		return _error(telnet, __LINE__, __func__, CRTL_TELNET_EBADVAL,
				err_fatal, "cannot initialize compression twice");

	/* allocate zstream box */
	if ((z= (z_stream *)calloc(1, sizeof(z_stream))) == 0)
		return _error(telnet, __LINE__, __func__, CRTL_TELNET_ENOMEM, err_fatal,
	...
	if (deflate) {
		if ((rs = deflateInit(z, Z_DEFAULT_COMPRESSION)) != Z_OK) {
			free(z);
			return _error(telnet, __LINE__, __func__, CRTL_TELNET_ECOMPRESS,
					err_fatal, "deflateInit() failed: %s", zError(rs));
		}
		telnet->flags |= CRTL_TELNET_PFLAG_DEFLATE;
	} else {
	...
	return CRTL_TELNET_EOK;
}
#endif /* defined(HAVE_ZLIB) */

/* push bytes out, compressing them first if need be */
static void _send(crtl_telnet_t *telnet, const char *buffer,
		size_t size) {
	crtl_telnet_event_t ev;

#if defined(HAVE_ZLIB)
	/* if we have a deflate (compression) zlib box, use it */
	if (telnet->z != 0 && telnet->flags & CRTL_TELNET_PFLAG_DEFLATE) {
		...
		while (telnet->z->avail_in > 0 || telnet->z->avail_out == 0) {
			...
			if ((rs = deflate(telnet->z, Z_SYNC_FLUSH)) != Z_OK) {
				_error(telnet, __LINE__, __func__, CRTL_TELNET_ECOMPRESS, 1,
 ...
 */
static INLINE int _check_telopt(crtl_telnet_t *telnet, unsigned char telopt,
		int us) {
	int i;
	...
	for (i = 0; telnet->telopts[i].telopt != -1; ++i) {
		if (telnet->telopts[i].telopt == telopt) {
			if (us && telnet->telopts[i].us == CRTL_TELNET_WILL)
				return 1;
			else if (!us && telnet->telopts[i].him == CRTL_TELNET_DO)
				return 1;
			else
				return 0;
		}
...
static INLINE crtl_telnet_rfc1143_t _get_rfc1143(crtl_telnet_t *telnet,
		unsigned char telopt) {
	crtl_telnet_rfc1143_t empty;
	unsigned int i;
...
static INLINE void _set_rfc1143(crtl_telnet_t *telnet, unsigned char telopt,
		char us, char him) {
	crtl_telnet_rfc1143_t *qtmp;
	unsigned int i;
	...
	for (i = 0; i != telnet->q_cnt; ++i) {
		if (telnet->q[i].telopt == telopt) {
			...
			if (telopt != CRTL_TELNET_TELOPT_BINARY)
				return;
			telnet->flags &= ~(CRTL_TELNET_FLAG_TRANSMIT_BINARY |
					   CRTL_TELNET_FLAG_RECEIVE_BINARY);
			if (us == Q_YES)
				telnet->flags |= CRTL_TELNET_FLAG_TRANSMIT_BINARY;
			if (him == Q_YES)
				telnet->flags |= CRTL_TELNET_FLAG_RECEIVE_BINARY;
			return;
		}
	}
	...
	if (telnet->q_cnt >= telnet->q_size) {
		/* Expand the size */
		if ((qtmp = (crtl_telnet_rfc1143_t *)realloc(telnet->q,
			sizeof(crtl_telnet_rfc1143_t) *
            	(telnet->q_size + Q_BUFFER_GROWTH_QUANTUM))) == 0) {
			_error(telnet, __LINE__, __func__, CRTL_TELNET_ENOMEM, 0,
					"realloc() failed: %s", strerror(errno));
			return;
		}
		memset(&qtmp[telnet->q_size], 0, sizeof(crtl_telnet_rfc1143_t) *
...
static INLINE void _send_negotiate(crtl_telnet_t *telnet, unsigned char cmd,
		unsigned char telopt) {
	unsigned char bytes[3];
	bytes[0] = CRTL_TELNET_IAC;
	bytes[1] = cmd;
	bytes[2] = telopt;
...
static void _negotiate(crtl_telnet_t *telnet, unsigned char telopt) {
	crtl_telnet_event_t ev;
	crtl_telnet_rfc1143_t q;

	/* in PROXY mode, just pass it thru and do nothing */
	if (telnet->flags & CRTL_TELNET_FLAG_PROXY) {
		switch ((int)telnet->state) {
		case CRTL_TELNET_STATE_WILL:
			NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_WILL, telopt);
			break;
		case CRTL_TELNET_STATE_WONT:
			NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_WONT, telopt);
			break;
		case CRTL_TELNET_STATE_DO:
			NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_DO, telopt);
			break;
		case CRTL_TELNET_STATE_DONT:
			NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_DONT, telopt);
			break;
		}
		return;
	}
	...
	case CRTL_TELNET_STATE_WILL:
		...
		case Q_NO:
			if (_check_telopt(telnet, telopt, 0)) {
				...
				_send_negotiate(telnet, CRTL_TELNET_DO, telopt);
				NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_WILL, telopt);
			} else
				_send_negotiate(telnet, CRTL_TELNET_DONT, telopt);
			break;
		case Q_WANTNO:
			_set_rfc1143(telnet, telopt, Q_US(q), Q_NO);
			NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_WONT, telopt);
			_error(telnet, __LINE__, __func__, CRTL_TELNET_EPROTOCOL, 0,
	...
	case CRTL_TELNET_STATE_DO:
		...
		case Q_NO:
			if (_check_telopt(telnet, telopt, 1)) {
				...
				_send_negotiate(telnet, CRTL_TELNET_WILL, telopt);
				NEGOTIATE_EVENT(telnet, CRTL_TELNET_EV_DO, telopt);
			} else
				_send_negotiate(telnet, CRTL_TELNET_WONT, telopt);
			break;
		case Q_WANTNO:
 ...
 */
static int _environ_telnet(crtl_telnet_t *telnet, unsigned char type,
		char* buffer, size_t size) {
	crtl_telnet_event_t ev;
	struct crtl_telnet_environ_t *values = 0;
	char *c, *last, *out;
	size_t index, count;
	...
	if ((unsigned)buffer[0] != CRTL_TELNET_ENVIRON_SEND &&
			(unsigned)buffer[0] != CRTL_TELNET_ENVIRON_IS &&
			(unsigned)buffer[0] != CRTL_TELNET_ENVIRON_INFO) {
		_error(telnet, __LINE__, __func__, CRTL_TELNET_EPROTOCOL, 0,
	...
	if (size == 1) {
		...
		ev.type = CRTL_TELNET_EV_ENVIRON;
		telnet->eh(telnet, &ev, telnet->ud);
	...
	if ((unsigned)buffer[1] != CRTL_TELNET_ENVIRON_VAR &&
			(unsigned)buffer[1] != CRTL_TELNET_ENVIRON_USERVAR) {
		_error(telnet, __LINE__, __func__, CRTL_TELNET_EPROTOCOL, 0,
	...
	if ((unsigned)buffer[size - 1] == CRTL_TELNET_ENVIRON_ESC) {
		_error(telnet, __LINE__, __func__, CRTL_TELNET_EPROTOCOL, 0,
	...
	for (c = buffer + 1; c < buffer + size; ++c) {
		if (*c == CRTL_TELNET_ENVIRON_VAR || *c == CRTL_TELNET_ENVIRON_USERVAR) {
			++count;
		} else if (*c == CRTL_TELNET_ENVIRON_ESC) {
			/* skip the next byte */
			++c;
		}
	}
	...
	if ((values = (struct crtl_telnet_environ_t *)calloc(count,
			sizeof(struct crtl_telnet_environ_t))) == 0) {
		_error(telnet, __LINE__, __func__, CRTL_TELNET_ENOMEM, 0,
	...
	for (index = 0; index != count; ++index) {
		...
		while (c < buffer + size) {
			...
			if ((unsigned)*c == CRTL_TELNET_ENVIRON_VAR ||
					(unsigned)*c == CRTL_TELNET_ENVIRON_VALUE ||
					(unsigned)*c == CRTL_TELNET_ENVIRON_USERVAR) {
				break;
			}

			/* buffer next byte (taking into account ESC) */
			if (*c == CRTL_TELNET_ENVIRON_ESC) {
				++c;
			}
		...
		*out++ = '\0';
		...
		if (c < buffer + size && *c == CRTL_TELNET_ENVIRON_VALUE) {
			++c;
			last = out;
			while (c < buffer + size) {
...
static int _mssp_telnet(crtl_telnet_t *telnet, char* buffer, size_t size) {
	crtl_telnet_event_t ev;
	struct crtl_telnet_environ_t *values;
	char *var = 0;
	char *c, *last, *out;
	size_t i, count;
	...
	if ((unsigned)buffer[0] != CRTL_TELNET_MSSP_VAR) {
		_error(telnet, __LINE__, __func__, CRTL_TELNET_EPROTOCOL, 0,
	...
	for (count = 0, i = 0; i != size; ++i) {
		if ((unsigned)buffer[i] == CRTL_TELNET_MSSP_VAL) {
			++count;
		}
	}
	...
	for (i = 0, c = buffer + 1; c < buffer + size;) {
		...
		while (c < buffer + size && (unsigned)*c != CRTL_TELNET_MSSP_VAR &&
				(unsigned)*c != CRTL_TELNET_MSSP_VAL) {
			*out++ = *c++;
		}
		*out++ = '\0';

		/* if it's a variable name, just store the name for now */
		if (next_type == CRTL_TELNET_MSSP_VAR) {
			var = last;
		} else if (next_type == CRTL_TELNET_MSSP_VAL && var != 0) {
			values[i].var = var;
			values[i].value = last;
			++i;
		} else {
...
static int _zmp_telnet(crtl_telnet_t *telnet, const char* buffer, size_t size) {
	crtl_telnet_event_t ev;
	char **argv;
	const char *c;
	size_t i, argc;
...
static int _ttype_telnet(crtl_telnet_t *telnet, const char* buffer, size_t size) {
	crtl_telnet_event_t ev;
	...
	if (buffer[0] != CRTL_TELNET_TTYPE_IS &&
			buffer[0] != CRTL_TELNET_TTYPE_SEND) {
		_error(telnet, __LINE__, __func__, CRTL_TELNET_EPROTOCOL, 0,
	...
	if (buffer[0] == CRTL_TELNET_TTYPE_IS) {
		...
		ev.type = CRTL_TELNET_EV_TTYPE;
		ev.ttype.cmd = CRTL_TELNET_TTYPE_IS;
		ev.ttype.name = name;
		telnet->eh(telnet, &ev, telnet->ud);
		...
		ev.type = CRTL_TELNET_EV_TTYPE;
		ev.ttype.cmd = CRTL_TELNET_TTYPE_SEND;
		ev.ttype.name = 0;
		telnet->eh(telnet, &ev, telnet->ud);
	}
 ...
 */
static int _subnegotiate(crtl_telnet_t *telnet) {
	crtl_telnet_event_t ev;

	/* standard subnegotiation event */
	ev.type = CRTL_TELNET_EV_SUBNEGOTIATION;
	 ...
	 */
	case CRTL_TELNET_TELOPT_COMPRESS2:
		if (telnet->sb_telopt == CRTL_TELNET_TELOPT_COMPRESS2) {
			if (_init_zlib(telnet, 0, 1) != CRTL_TELNET_EOK)
				return 0;

			/* notify app that compression was enabled */
			ev.type = CRTL_TELNET_EV_COMPRESS;
			ev.compress.state = 1;
			telnet->eh(telnet, &ev, telnet->ud);
			return 1;
		}
...
crtl_telnet_t *crtl_telnet_init(const crtl_telnet_telopt_t *telopts,
		crtl_telnet_event_handler_t eh, unsigned char flags, void *user_data) {
	/* allocate structure */
	struct crtl_telnet_t *telnet = (crtl_telnet_t*)calloc(1, sizeof(crtl_telnet_t));
	if (telnet == 0)
		return 0;
...
void crtl_telnet_free(crtl_telnet_t *telnet) {
	...
	/* free zlib box */
	if (telnet->z != 0) {
		if (telnet->flags & CRTL_TELNET_PFLAG_DEFLATE)
			deflateEnd(telnet->z);
		else
			inflateEnd(telnet->z);
		free(telnet->z);
...
static crtl_telnet_error_t _buffer_byte(crtl_telnet_t *telnet,
		unsigned char byte) {
	char *new_buffer;
	size_t i;
	...
	if (telnet->buffer_pos == telnet->buffer_size) {
		...
		if (i >= _buffer_sizes_count - 1) {
			_error(telnet, __LINE__, __func__, CRTL_TELNET_EOVERFLOW, 0,
					"subnegotiation buffer size limit reached");
			return CRTL_TELNET_EOVERFLOW;
		}
...
static void _process(crtl_telnet_t *telnet, const char *buffer, size_t size) {
	crtl_telnet_event_t ev;
	unsigned char byte;
	size_t i, start;
	for (i = start = 0; i != size; ++i) {
		...
		switch (telnet->state) {
		/* regular data */
		case CRTL_TELNET_STATE_DATA:
			/* on an IAC byte, pass through all pending bytes and
			 * switch states */
			if (byte == CRTL_TELNET_IAC) {
				if (i != start) {
					ev.type = CRTL_TELNET_EV_DATA;
					ev.data.buffer = buffer + start;
					ev.data.size = i - start;
					telnet->eh(telnet, &ev, telnet->ud);
				}
				telnet->state = CRTL_TELNET_STATE_IAC;
			} else if (byte == '\r' &&
					   (telnet->flags & CRTL_TELNET_FLAG_NVT_EOL) &&
					   !(telnet->flags & CRTL_TELNET_FLAG_RECEIVE_BINARY)) {
				if (i != start) {
					ev.type = CRTL_TELNET_EV_DATA;
					ev.data.buffer = buffer + start;
				...
				telnet->state = CRTL_TELNET_STATE_EOL;
			}
			break;

		/* NVT EOL to be translated */
		case CRTL_TELNET_STATE_EOL:
			if (byte != '\n') {
				byte = '\r';
				ev.type = CRTL_TELNET_EV_DATA;
			...
			if (byte == '\0')
				++start;
			/* state update */
			telnet->state = CRTL_TELNET_STATE_DATA;
			break;

		/* IAC command */
		case CRTL_TELNET_STATE_IAC:
			switch (byte) {
			/* subnegotiation */
			case CRTL_TELNET_SB:
				telnet->state = CRTL_TELNET_STATE_SB;
				break;
			/* negotiation commands */
			case CRTL_TELNET_WILL:
				telnet->state = CRTL_TELNET_STATE_WILL;
				break;
			case CRTL_TELNET_WONT:
				telnet->state = CRTL_TELNET_STATE_WONT;
				break;
			case CRTL_TELNET_DO:
				telnet->state = CRTL_TELNET_STATE_DO;
				break;
			case CRTL_TELNET_DONT:
				telnet->state = CRTL_TELNET_STATE_DONT;
				break;
			/* IAC escaping */
			case CRTL_TELNET_IAC:
				/* event */
				ev.type = CRTL_TELNET_EV_DATA;
				ev.data.buffer = (char*)&byte;
				ev.data.size = 1;
		...
		case CRTL_TELNET_STATE_SB:
			telnet->sb_telopt = byte;
			telnet->buffer_pos = 0;
			telnet->state = CRTL_TELNET_STATE_SB_DATA;
			break;

		/* subnegotiation -- buffer bytes until end request */
		case CRTL_TELNET_STATE_SB_DATA:
			/* IAC command in subnegotiation -- either IAC SE or IAC IAC */
			if (byte == CRTL_TELNET_IAC) {
				telnet->state = CRTL_TELNET_STATE_SB_DATA_IAC;
			} else if (telnet->sb_telopt == CRTL_TELNET_TELOPT_COMPRESS && byte == CRTL_TELNET_WILL) {
			...
			} else if (_buffer_byte(telnet, byte) != CRTL_TELNET_EOK) {
				start = i + 1;
				telnet->state = CRTL_TELNET_STATE_DATA;
			}
			break;
		...
		case CRTL_TELNET_STATE_SB_DATA_IAC:
			switch (byte) {
			/* end subnegotiation */
			case CRTL_TELNET_SE:
				...
				if (_subnegotiate(telnet) != 0) {
					 ...
					 * we have to re-invoke crtl_telnet_recv to get those
					 * bytes inflated and abort trying to process the
					 * remaining compressed bytes in the current _process
					 * buffer argument
					 */
					crtl_telnet_recv(telnet, &buffer[start], size - start);
					return;
				}
				break;
			/* escaped IAC byte */
			case CRTL_TELNET_IAC:
				...
				if (_buffer_byte(telnet, CRTL_TELNET_IAC) !=
						CRTL_TELNET_EOK) {
					start = i + 1;
					telnet->state = CRTL_TELNET_STATE_DATA;
				} else {
			...
			default:
				 ...
				 * CRTL_TELNET_STATE_SB_DATA_IAC about invoking crtl_telnet_recv()
				 */
				if (_subnegotiate(telnet) != 0) {
					crtl_telnet_recv(telnet, &buffer[start], size - start);
					return;
	...
	if (telnet->state == CRTL_TELNET_STATE_DATA && i != start) {
		ev.type = CRTL_TELNET_EV_DATA;
		ev.data.buffer = buffer + start;
		ev.data.size = i - start;
		telnet->eh(telnet, &ev, telnet->ud);
...
void crtl_telnet_recv(crtl_telnet_t *telnet, const char *buffer,	size_t size) 
{
#if defined(HAVE_ZLIB)
...
/* send an iac command */
void crtl_telnet_iac(crtl_telnet_t *telnet, unsigned char cmd) {
...
/* send negotiation */
void crtl_telnet_negotiate(crtl_telnet_t *telnet, unsigned char cmd,
		unsigned char telopt) {
	crtl_telnet_rfc1143_t q;
...
void crtl_telnet_send(crtl_telnet_t *telnet, const char *buffer,
		size_t size) {
	size_t i, l;

	for (l = i = 0; i != size; ++i) {
		/* dump prior portion of text, send escaped bytes */
		if (buffer[i] == (char)CRTL_TELNET_IAC) {
			...
			/* send escape */
			crtl_telnet_iac(telnet, CRTL_TELNET_IAC);
		}
	}
...
void crtl_telnet_send_text(crtl_telnet_t *telnet, const char *buffer,
		size_t size) {
	size_t i, l;
	...
	for (l = i = 0; i != size; ++i) {
		...
		else if (!(telnet->flags & CRTL_TELNET_FLAG_TRANSMIT_BINARY) &&
				 (buffer[i] == '\r' || buffer[i] == '\n')) {
...
void crtl_telnet_begin_sb(crtl_telnet_t *telnet, unsigned char telopt) {
	unsigned char sb[3];
	sb[0] = CRTL_TELNET_IAC;
	sb[1] = CRTL_TELNET_SB;
	sb[2] = telopt;
	_sendu(telnet, sb, 3);
}
...
void crtl_telnet_subnegotiation(crtl_telnet_t *telnet, unsigned char telopt,
		const char *buffer, size_t size) {
	unsigned char bytes[5];
	bytes[0] = CRTL_TELNET_IAC;
	bytes[1] = CRTL_TELNET_SB;
	bytes[2] = telopt;
	bytes[3] = CRTL_TELNET_IAC;
	bytes[4] = CRTL_TELNET_SE;
	 ...
	 */
	if (telnet->flags & CRTL_TELNET_FLAG_PROXY &&
			telopt == CRTL_TELNET_TELOPT_COMPRESS2) {
		crtl_telnet_event_t ev;

		if (_init_zlib(telnet, 1, 1) != CRTL_TELNET_EOK)
			return;
...
void crtl_telnet_begin_compress2(crtl_telnet_t *telnet) {
#if defined(HAVE_ZLIB)
	static const unsigned char compress2[] = { CRTL_TELNET_IAC, CRTL_TELNET_SB,
			CRTL_TELNET_TELOPT_COMPRESS2, CRTL_TELNET_IAC, CRTL_TELNET_SE };

	crtl_telnet_event_t ev;

	/* attempt to create output stream first, bail if we can't */
	if (_init_zlib(telnet, 1, 0) != CRTL_TELNET_EOK)
		return;
...
int crtl_telnet_vprintf(crtl_telnet_t *telnet, const char *fmt, va_list va) {
	va_list va_temp;
	char buffer[1024];
	char *output = buffer;
	unsigned int rs, i, l;
	...
	for (l = i = 0; i != rs; ++i) {
		/* special characters */
		if (output[i] == (char)CRTL_TELNET_IAC || output[i] == '\r' ||
				output[i] == '\n') {
			/* dump prior portion of text */
			if (i != l)
			...
			/* IAC -> IAC IAC */
			if (output[i] == (char)CRTL_TELNET_IAC)
				crtl_telnet_iac(telnet, CRTL_TELNET_IAC);
			/* automatic translation of \r -> CRNUL */
			else if (output[i] == '\r')
				_send(telnet, CRNUL, 2);
...
/* see crtl_telnet_vprintf */
int crtl_telnet_printf(crtl_telnet_t *telnet, const char *fmt, ...) {
	va_list va;
	int rs;

	va_start(va, fmt);
	rs = crtl_telnet_vprintf(telnet, fmt, va);
	va_end(va);

	return rs;
}

/* send formatted data through crtl_telnet_send */
int crtl_telnet_raw_vprintf(crtl_telnet_t *telnet, const char *fmt, va_list va) {
	va_list va_temp;
	char buffer[1024];
	char *output = buffer;
	unsigned int rs;
	...
	crtl_telnet_send(telnet, output, rs);
...
/* see crtl_telnet_raw_vprintf */
int crtl_telnet_raw_printf(crtl_telnet_t *telnet, const char *fmt, ...) {
	va_list va;
	int rs;

	va_start(va, fmt);
	rs = crtl_telnet_raw_vprintf(telnet, fmt, va);
	va_end(va);
...
void crtl_telnet_begin_newenviron(crtl_telnet_t *telnet, unsigned char cmd) {
	crtl_telnet_begin_sb(telnet, CRTL_TELNET_TELOPT_NEW_ENVIRON);
	crtl_telnet_send(telnet, (const char *)&cmd, 1);
}

/* send a NEW-ENVIRON value */
void crtl_telnet_newenviron_value(crtl_telnet_t *telnet, unsigned char type,
		const char *string) {
	crtl_telnet_send(telnet, (const char*)&type, 1);

	if (string != 0) {
		crtl_telnet_send(telnet, string, strlen(string));
	}
}

/* send TERMINAL-TYPE SEND command */
void crtl_telnet_ttype_send(crtl_telnet_t *telnet) {
    static const unsigned char SEND[] = { CRTL_TELNET_IAC, CRTL_TELNET_SB,
			CRTL_TELNET_TELOPT_TTYPE, CRTL_TELNET_TTYPE_SEND, CRTL_TELNET_IAC, CRTL_TELNET_SE };
	_sendu(telnet, SEND, sizeof(SEND));
}

/* send TERMINAL-TYPE IS command */
void crtl_telnet_ttype_is(crtl_telnet_t *telnet, const char* ttype) {
	static const unsigned char IS[] = { CRTL_TELNET_IAC, CRTL_TELNET_SB,
			CRTL_TELNET_TELOPT_TTYPE, CRTL_TELNET_TTYPE_IS };
...
/* send ZMP data */
void crtl_telnet_send_zmp(crtl_telnet_t *telnet, size_t argc, const char **argv) {
	size_t i;

	/* ZMP header */
	crtl_telnet_begin_zmp(telnet, argv[0]);

	/* send out each argument, including trailing NUL byte */
	for (i = 1; i != argc; ++i)
		crtl_telnet_zmp_arg(telnet, argv[i]);

	/* ZMP footer */
	crtl_telnet_finish_zmp(telnet);
}

/* send ZMP data using varargs  */
void crtl_telnet_send_vzmpv(crtl_telnet_t *telnet, va_list va) {
	const char* arg;

	/* ZMP header */
	crtl_telnet_begin_sb(telnet, CRTL_TELNET_TELOPT_ZMP);

	/* send out each argument, including trailing NUL byte */
	while ((arg = va_arg(va, const char *)) != 0)
		crtl_telnet_zmp_arg(telnet, arg);

	/* ZMP footer */
	crtl_telnet_finish_zmp(telnet);
}

/* see crtl_telnet_send_vzmpv */
void crtl_telnet_send_zmpv(crtl_telnet_t *telnet, ...) {
	va_list va;

	va_start(va, telnet);
	crtl_telnet_send_vzmpv(telnet, va);
	va_end(va);
}

/* begin a ZMP command */
void crtl_telnet_begin_zmp(crtl_telnet_t *telnet, const char *cmd) {
	crtl_telnet_begin_sb(telnet, CRTL_TELNET_TELOPT_ZMP);
	crtl_telnet_zmp_arg(telnet, cmd);
}

/* send a ZMP argument */
void crtl_telnet_zmp_arg(crtl_telnet_t *telnet, const char* arg) {
	crtl_telnet_send(telnet, arg, strlen(arg) + 1);
}
