...
#define BITS_PER_XA_VALUE	(CRTL_BITS_PER_LONG - 1)
...
#define XARRAY_INIT(name, flags) {				\
	.xa_lock = CRTL_LOCK_MUTEX_INITIALIZER,		\
	.xa_flags = flags,					\
	.xa_head = NULL,					\
}
...
{
	crtl_mutex_init(&xa->xa_lock, NULL);
	xa->xa_flags = flags;
	xa->xa_head = NULL;
}
...
#define xa_trylock(xa)		crtl_mutex_lock(&(xa)->xa_lock, 1,0,0,0)
#define xa_lock(xa)		    crtl_mutex_lock(&(xa)->xa_lock, 0,0,0,0)
#define xa_unlock(xa)		crtl_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_bh(xa)		crtl_mutex_lock(&(xa)->xa_lock, 0,0,0,0)
#define xa_unlock_bh(xa)	crtl_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_irq(xa)		crtl_mutex_lock(&(xa)->xa_lock, 0,0,0,0)
#define xa_unlock_irq(xa)	crtl_mutex_unlock(&(xa)->xa_lock)
#define xa_lock_irqsave(xa, flags) \
				crtl_mutex_lock(&(xa)->xa_lock, 0,0,0,0)
#define xa_unlock_irqrestore(xa, flags) \
				crtl_mutex_unlock(&(xa)->xa_lock)
...
int __xa_insert(struct xarray *, unsigned long index,
		void *entry, gfp_t);
int __xa_alloc(struct xarray *, unsigned int *id, void *entry,
		struct xa_limit, gfp_t);
int __xa_alloc_cyclic(struct xarray *, unsigned int *id, void *entry,
		struct xa_limit, unsigned int *next, gfp_t);
 ...
 */
static inline int xa_alloc(struct xarray *xa, unsigned int *id,
 ...
 * -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_bh(struct xarray *xa, unsigned int *id,
		void *entry, struct xa_limit limit, gfp_t gfp)
{
	int err;
 ...
 * -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_irq(struct xarray *xa, unsigned int *id,
		void *entry, struct xa_limit limit, gfp_t gfp)
{
	int err;
 ...
 */
static inline int xa_alloc_cyclic(struct xarray *xa, unsigned int *id, void *entry,
		struct xa_limit limit, unsigned int *next, gfp_t gfp)
{
	int err;
 ...
 * allocated or -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_cyclic_bh(struct xarray *xa, unsigned int *id, void *entry,
		struct xa_limit limit, unsigned int *next, gfp_t gfp)
{
	int err;
 ...
 * allocated or -EBUSY if there are no free entries in @limit.
 */
static inline int xa_alloc_cyclic_irq(struct xarray *xa, unsigned int *id, void *entry,
		struct xa_limit limit, unsigned int *next, gfp_t gfp)
{
	int err;
 ...
 * Return: 0 if the reservation succeeded or -ENOMEM if it failed.
 */
static inline int xa_reserve_bh(struct xarray *xa, unsigned long index, gfp_t gfp)
{
 ...
 * Return: 0 if the reservation succeeded or -ENOMEM if it failed.
 */
static inline int xa_reserve_irq(struct xarray *xa, unsigned long index, gfp_t gfp)
{
...
#define XA_MARK_LONGS		DIV_ROUND_UP(XA_CHUNK_SIZE, CRTL_BITS_PER_LONG)
					...
					unsigned int order)
{
	...
	xas->xa_index = order < CRTL_BITS_PER_LONG ? (index >> order) << order : 0;
		...
		xa_mark_t mark)
{
	...
	if (advance)
		offset++;
	if (XA_CHUNK_SIZE == CRTL_BITS_PER_LONG) {
	...
	return crtl_find_next_bit(addr, XA_CHUNK_SIZE, offset);
}
