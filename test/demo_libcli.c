...
int cmd_test(struct crtl_cli_struct *cli, const char *command, char *argv[], int argc) {
  ...
  for (i = 0; i < argc; i++) crtl_cli_print(cli, "        %s", argv[i]);
...
int check_auth(const char *username, const char *password) {
  if (strcasecmp(username, "root") != 0) return CLI_ERROR;
  if (strcasecmp(password, "123456") != 0) return CLI_ERROR;
  return CLI_OK;
}
...
int cmd_perimeter(struct crtl_cli_struct *cli, const char *command, char *argv[], int argc) {
  ...
  for (; optargs; optargs = optargs->next) crtl_cli_print(cli, "%d, %s=%s", i++, optargs->name, optargs->value);
